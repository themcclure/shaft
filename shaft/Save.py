...
def create_results(file_name, officials, model):
    ...
    wb['model'].append(['Age:'] + list(range(len(model.decay))) + ['+'])
    ...
    for assn in sorted(model.wgt, reverse=True):
        ...
        # print out each game type in the association model
...
def create_raw_results(file_name, officials, model):
    ...
    for off in sorted(officials, key=attrgetter('name')):
        ...
        for j in off.games:
            ...
            # print(f"name = {game_denormalized[0]}, role = {game_denormalized[5]}, on {game_denormalized[3]}")
